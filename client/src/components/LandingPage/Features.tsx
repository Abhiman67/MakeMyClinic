...
const Features = () => {
  const coreFeatures = [
    ...
    {
      icon: <FaBed className="w-12 h-12 text-amber-500" />,
  ...
  return (
    <section id="features" className="py-24 relative bg-gray-50 dark:bg-yellow-950">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          {coreFeatures.map((feature, index) => (
            <Slide key={index} direction={index % 2 === 0 ? "left" : "right"} triggerOnce delay={index * 100}>
              <motion.div 
                className="bg-white dark:bg-yellow-900/30 backdrop-blur-sm rounded-2xl overflow-hidden border border-yellow-100 dark:border-yellow-500/20 h-full flex flex-col shadow-sm hover:shadow-xl transition-all duration-300"
                whileHover={{ 
                  y: -10, 
              ...
              >
                ...
                <div className="p-8 flex-1 flex flex-col">
                  ...
                  <p className="text-gray-700 dark:text-yellow-200 flex-1">{feature.description}</p>
                  
                  <motion.button 
                    className="mt-6 text-yellow-600 dark:text-yellow-400 font-medium flex items-center group"
                    whileHover={{ x: 5 }}
                  >
                    Learn more 
        ...
        <div className="mt-24 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            <Fade key={index} direction="up" triggerOnce delay={index * 100}>
              <div className="text-center p-6 bg-white dark:bg-yellow-900/30 backdrop-blur-sm rounded-xl border border-yellow-100 dark:border-yellow-500/20 shadow-sm">
                <h3 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">{stat.value}</h3>
                <p className="text-gray-700 dark:text-yellow-300">{stat.label}</p>
              </div>
            </Fade>
          ))}
        </div>
