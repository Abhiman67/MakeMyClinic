  ...
  const handleSubmit = (e?: React.MouseEvent) => {
    ...
    if (patientName && patientGender && selectedBed) {
      const newPatient: Patient = {
        ...
        status: "Occupied",
        serial: `OPD-${Math.floor(Math.random() * 1000)}`,
        contact: "1234567890",
      ...
      <div className="bg-gray-800 text-white w-96 p-6 rounded-lg shadow-lg">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            className="w-full mt-2 p-2 rounded bg-gray-700 border-none focus:ring-2 focus:ring-amber-500"
        ...
        <div className="flex justify-end space-x-4">
          <button type="button" onClick={(e) => { e.preventDefault(); onClose(); }} className="px-4 py-2 bg-gray-600 rounded hover:bg-gray-700 transition">
            Cancel
          </button>
          <button type="button" onClick={handleSubmit} className="px-4 py-2 bg-amber-600 text-white rounded hover:bg-amber-700 transition">
            Save
          </button>
        </div>
      </div>
